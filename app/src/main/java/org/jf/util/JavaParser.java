...
public class JavaParser
{
	...
	public JavaParser(ClassDataItem.EncodedMethod method)
	{
		this.method = method;
	    ...
	    //Ljava/lang/String; -> String
         public static String typeDesToName(String l)
		 {
		...
		public void dumpJava(IndentingWriter2 writer)throws IOException{
			 ...
			 MethodIdItem methodid=  method.method;
			 
			//writer.write("#");
			writer.write( AccessFlags.formatAccessFlagsForClass( method.accessFlags));
			writer.write(" ");
			
			writer.write(methodid.getPrototype().getReturnType().getTypeDescriptor());
			writer.write(" ");
			
			writer.write(methodid.getMethodName().getStringValue());
			writer.write(methodid.getPrototype().getPrototypeString());
			writer.write("\n{");
			
			
			//显示寄存器的类型
			int register = code.getRegisterCount();
			TypeListItem paraList=   method.method.getPrototype().getParameters();
		
			int nowPara=0;
			if(paraList!=null)
			for(TypeIdItem para:paraList.getTypeList())
			{
				writer.write("\n# v"+(register-1)+" ="+typeDesToName(  para.getTypeDescriptor())+ String.valueOf(nowPara));
				register-=1;
				nowPara+=1;
			}
			writer.write("\n");
...
//						writer.printIntAsDec(ins11x.getRegisterA());
						String s;
						if(instruction.opcode.name.indexOf("return")!=-1)
							s=instruction.opcode.name+"v"+ins11x.getRegisterA();
							else
						s= "v"+ins11x.getRegisterA()+"="+writer.getLastLine();
						writer.write(s);
						writer.write('\n');
						break;
					...
					case Format11n:
						...
						writer.write('\n');
						break;
						//move move-wide,array length
					case Format12x:
						Instruction12x ins12x=(Instruction12x)instruction;
						
						if(instruction.opcode.fakename.indexOf("=")==-1)
						{
						writer.write('v');
						...
						writer.write(instruction.opcode.fakename);
		...
		private static void writeToReferencedItem(IndentingWriter2 writer,Instruction instruction)throws IOException {
			...
			switch (instruction.opcode.referenceType) {
				case field:
					...
					writer.write( typeDesToName( field.classType.getTypeDescriptor())+"."
				...
				case method:
					...
					writer.write( typeDesToName( method.classType.getTypeDescriptor())+"."
